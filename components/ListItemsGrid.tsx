...
import EndedSeriesFilter from './EndedSeriesFilter';
...
export default function ListItemsGrid({ listId, items }: ListItemsGridProps) {
	...
	const isEnded = (item: ListItem) =>
		item.media_type === 'tv' &&
		isEndedToFinish(item.status, getWatchedCount(item.id), item.totalEpisodes);

	const endedCount = useMemo(
		() => items.filter(isEnded).length,
		// eslint-disable-next-line react-hooks/exhaustive-deps
		[items, getWatchedCount]
	);

	const visibleItems = endedOnly ? items.filter(isEnded) : items;
		...
		<div>
			...
			{visibleItems.length === 0 ? (
				<div className="text-center py-16 bg-white/5 rounded-2xl border border-white/10 border-dashed">
					...
					<p className="text-gray-400">{t('following.no_ended')}</p>
				</div>
			) : (
